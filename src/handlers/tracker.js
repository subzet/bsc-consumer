...
const trackSwaps = async (tokenAddress) => {
    ...
    const references = Object.keys(config.get('addresses')).map(key => { return config.get('addresses')[key] })
    const tracker = new RouterTracker(routerAddress,tokenAddress,functions,references)
    
    const contract = await getTokenContract(tokenAddress)
    const token = await new Token(contract,tracker.router,config.get('addresses')['BUSD'])
    tokens.push(token)

    const busdContract = await getTokenContract(config.get('addresses')['BUSD'])
    const busdToken = await new Token(busdContract,tracker.router,config.get('addresses')['BUSD'])
    tokens.push(busdToken)

    const wbnbContract = await getTokenContract(config.get('addresses')['WBNB'])
    const wbnbToken = await new Token(wbnbContract,tracker.router,config.get('addresses')['BUSD'])
    tokens.push(wbnbToken)

    tracker.track()

    tracker.on('transaction', async function (transaction){
        ...
        try{
            ...
            //Emitir evento transacción para que lo agarre el haandler de candles.
            console.log(
                color,
                `[${moment().format()}]`,
                label, 
